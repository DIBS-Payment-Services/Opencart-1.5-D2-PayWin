      ...
      <div id="tab-payment" class="vtabs-content">
        <table class="form">
          ...
          <tr>
            ...
            $dibsTableExists = false;
            foreach($result->rows as $key=>$value) {
                $result = array_values($value);    
            if( strpos($result[0],"dibs_pw_results") !== false) {
                $dibsTableExists = true;
                break;    
            }
          }
          $addFields = array();
          if($dibsTableExists) {
              $acquirerFileds = array("acquirerLastName"=> "Acquirer Laet Name", 
                                      "acquirerFirstName"=>"Acquirer First Name",
                                      "acquirerDeliveryAddress" => "Acquirer Delivery Address" ,
                                      "acquirerDeliveryPostalCode"=>"Acquirer Delivery Postal Code",
                                      "acquirerDeliveryPostalPlace"=> "Acquirer Delivery Postal Place");
              $result = $this->db->query("SELECT `ext_info` FROM `" . DB_PREFIX . "dibs_pw_results` WHERE `orderid` = {$order_id}");
              
              $resArr = unserialize(($result->row['ext_info']));          
           
              if($resArr) {
              foreach($resArr as $key => $value){
                  if( array_key_exists($key, $acquirerFileds)) {
                      $addFields[$key] = $value;
                  }
              }
              $out = "";
